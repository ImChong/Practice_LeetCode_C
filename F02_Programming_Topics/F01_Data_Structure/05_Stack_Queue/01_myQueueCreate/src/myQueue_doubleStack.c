...
void in2out(MyQueue *obj) {
    while (!Stack_IsEmpty(obj->inStack)) {                     /* 当入队栈不为空时 */
...
int myQueuePop(MyQueue *obj) {
    if (Stack_IsEmpty(obj->outStack)) {                        /* 如果出队栈为空 */
...
bool myQueueEmpty(MyQueue* obj) {
    return (Stack_IsEmpty(obj->inStack) && Stack_IsEmpty(obj->outStack));     /* 返回入队栈和出队栈是否都为空 */
}
