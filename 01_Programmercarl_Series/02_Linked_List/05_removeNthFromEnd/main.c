...
void test_1(void) {
    ...
    struct ListNode *newList = linkedListFromArray(arr, arrSize);
    int n = 2;
    ...
    struct ListNode *expectList = linkedListFromArray(expectArr, expectArrSize);
...
void test_4(void) {
    ...
    /* 释放内存 */
    freeList(ansList);        /* newList 的链表头已被释放内存 */
    freeList(expectList);
}
